...
### Carry-forward from Plan 1 (Foundation) — final whole-branch review

Plan 1 merged clean (8 commits `45e1942..203bb4a`; full non-docker/gpu suite green, mypy clean). The final review flagged items to resolve **when Plan 2 starts** (none blocked Plan 1):

1. **Sequence `x-autobio` hint placement (decide first).** `GenericSequenceSet`/`AntibodySequenceSet` are `Annotated[canonical, BeforeValidator]` whose *declared* JSON Schema is just `object` — so the `x-autobio: {widget: "sequence", flavor: ...}` hint is the ONLY signal that FASTA text/file is accepted. Decide: bundle the hint into each alias (`Annotated[..., Field(json_schema_extra=ui(widget=Widget.SEQUENCE, flavor="generic"|"antibody"))]`, documenting that a per-field `Field` may override it) vs. a per-field checklist item. Forgetting it silently drops the FASTA affordance.
2. **Registry naming reconciliation.** Implementation ships `CATALOG` in `core/catalog.py` (intentional, to coexist with the live `TOOL_REGISTRY` in `core/registry.py`). Make the eventual public name + home (`TOOL_REGISTRY` in `core/registry.py` per spec §4, or keep `catalog.py`) an explicit Plan 2/3 step so consumers don't import a soon-to-move symbol.
3. **Phase-1 polish sweep** (do while touching these modules for the first family migration): `get_category_info` friendly error like `get_tool` (fail-fast preference); `ui()` `group`/`enum_labels` coverage; antibody 4th-error-path (non-protein) + missing-file tests; empty-input enforced via per-mode `Field(min_length=1)` (NOT in the SequenceSet alias); distinguish empty-sequence from non-protein in the antibody FASTA error message; `Args:` docstrings + `-> None` on `test_catalog.py`/`test_hints.py`; type the `_HEAVY_TOKENS`/`_LIGHT_TOKENS` constants; note that generic FASTA (unlike antibody) does not residue-validate.
