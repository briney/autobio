...
- **Plan 4 — remaining family migrations (Phase 1 cont.).** Migrate the rest against the now-hardened recipe, grouped by shape (suggested PR-per-group, simple → complex):
  - **First recipe item (from Plan 3 final review):** strengthen `ToolRunner._apply_extra` to also reject `extra` keys that collide with *runner-derived* `config` keys (not just typed input fields) — i.e. reject `set(input_data.extra) & set(config)` at call time in addition to the typed-field check. Today the guard covers typed input fields only, so a caller could still pass `extra={"output_dir": ...}` and silently overwrite a container-contract path via `config.update`. This is strictly more protective and breaks no current test; `ligandmpnn_packer.py:113` already uses the stricter `key not in config` form. Update the error message to name both collision sources and the three shadow-test `match=` strings (`test_tool_runner_modes.py`, `test_freesasa.py`, `test_esm.py`). Do this before migrating families so all 11 inherit the strongest guard.
