...
Simple protein predictions use the ``sequences`` dict on ``BoltzInput``. For
multi-entity predictions (DNA, RNA, ligands) or advanced features (constraints,
modifications), agents either specify ``entity_types`` or provide a raw Boltz
YAML via ``boltz_yaml``.

CLI-level args (``sampling_steps``, ``step_scale``, etc.) are passed through
the ``extra`` dict on ``BoltzInput``.
"""
...
class BoltzRunner(ToolRunner):
    ...
    standardised ``BoltzInput`` fields and writes ``config.json``.
    ...
    def prepare_workspace(self, input_data: BaseInput, workspace: Workspace) -> None:
        ...
        assert isinstance(input_data, BoltzInput)

        # -- Host-side validation (fail fast before container launch) --------
        self._validate_inputs(input_data)

        # -- Generate or pass through Boltz YAML input ----------------------
        if input_data.boltz_yaml is not None:
            yaml_data = input_data.boltz_yaml
    ...
    def _build_boltz_yaml(self, input_data: BoltzInput, workspace: Workspace) -> dict[str, Any]:
        ...
        entity type is ``protein``; override per-chain via ``entity_types``.
        ...
        # Add optional sections from typed fields
...
_BOLTZ_SHARED_NOTES = (
    ...
    "'templates' field on BoltzInput. Files are copied into the workspace and "
    "paths are rewritten automatically.",
    # MSA options
    ...
    "set the 'use_msa_server' field to false. Alternatively, provide "
    "pre-computed MSAs via the 'msa_paths' field (list of file paths, "
    "filenames should start with the chain ID, e.g., 'A.a3m').",
    # Key parameters
