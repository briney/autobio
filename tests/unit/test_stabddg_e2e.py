...
The only thing not tested is the actual StaB-ddG model execution.
...
# ---------------------------------------------------------------------------
# Realistic StaB-ddG output data
# ---------------------------------------------------------------------------
...
# Single-trial output CSV (default trials=1)
_SINGLE_TRIAL_CSV = "Name,Mutation,pred_1\n1AO7_A_B,EA63Q,-0.542\n"

# Multi-trial output CSV (trials=3)
_MULTI_TRIAL_CSV = "Name,Mutation,pred_1,pred_2,pred_3\n1AO7_A_B,EA63Q,-0.542,-0.601,-0.489\n"

# Multiple mutations combined
_MULTI_MUTATION_CSV = 'Name,Mutation,pred_1\n1AO7_ABC_DE,"YH103H,QD30V",1.230\n'

# Multiple rows (batch-like output)
_MULTI_ROW_CSV = "Name,Mutation,pred_1\n1AO7_A_B,EA63Q,-0.542\n1AO7_A_B,KA66A,0.123\n"
...
def _make_runner(config: AutobioConfig) -> StaBddGRunner:
...
def _import_standardize():
    ...
    container_dir = str(Path(__file__).resolve().parent.parent.parent / "containers" / "stabddg")
...
# ---------------------------------------------------------------------------
# TestStaBddGSingleTrialE2E
# ---------------------------------------------------------------------------


class TestStaBddGSingleTrialE2E:
    """End-to-end test for single-trial StaB-ddG prediction."""

    def test_single_mutation_pipeline(
        self, config: AutobioConfig, complex_pdb: Path, tmp_path: Path
    ) -> None:
        """Single mutation, single trial — basic pipeline."""
        input_data = ScoringInput(
            structure_path=complex_pdb,
            extra={"mutations": ["EA63Q"], "chains": "A_B"},
        )
        output = _run_e2e(config, input_data, _SINGLE_TRIAL_CSV, tmp_path)
...
# ---------------------------------------------------------------------------
# TestStaBddGMultiTrialE2E
# ---------------------------------------------------------------------------


class TestStaBddGMultiTrialE2E:
    """End-to-end test for multi-trial StaB-ddG prediction."""

    def test_multi_trial_averages(
        self, config: AutobioConfig, complex_pdb: Path, tmp_path: Path
    ) -> None:
        """Multi-trial output averages ddG across trials."""
        input_data = ScoringInput(
            structure_path=complex_pdb,
            extra={"mutations": ["EA63Q"], "chains": "A_B", "trials": 3},
        )
        output = _run_e2e(config, input_data, _MULTI_TRIAL_CSV, tmp_path)
    ...
    def test_multi_trial_breakdown(
        self, config: AutobioConfig, complex_pdb: Path, tmp_path: Path
    ) -> None:
        """Multi-trial output includes trial values in breakdown."""
        input_data = ScoringInput(
            structure_path=complex_pdb,
            extra={"mutations": ["EA63Q"], "chains": "A_B", "trials": 3},
        )
        ...
        assert breakdown["n_trials"] == 3
        assert "trial_values" in breakdown
        assert breakdown["trial_values"]["pred_1"] == pytest.approx(-0.542)


# ---------------------------------------------------------------------------
# TestStaBddGMultiMutationE2E
# ---------------------------------------------------------------------------


class TestStaBddGMultiMutationE2E:
...
# ---------------------------------------------------------------------------
# TestStaBddGMultiRowE2E
# ---------------------------------------------------------------------------


class TestStaBddGMultiRowE2E:
...
# ---------------------------------------------------------------------------
# TestStaBddGConfigE2E
# ---------------------------------------------------------------------------


class TestStaBddGConfigE2E:
