#!/usr/bin/env bash
# validate_config.sh — Phase 1: verify config.json for AntiBERTa2 tools
#
...
if [ -n "$MODEL_NAME" ] && [ "$MODEL_NAME" != "null" ]; then
    case "$MODEL_NAME" in
        alchemab/antiberta2)
            ;;
        *)
            error "Unknown model_name: $MODEL_NAME (expected 'alchemab/antiberta2')"
            ;;
    esac

    # Verify the model exists in the HuggingFace cache
    HF_CACHE=$(jq -r '.hf_cache // "/app/antiberta2/hf_cache"' "$CONFIG_FILE")
...
if [ -n "$CHAIN_SEP" ] && [ "$CHAIN_SEP" != "null" ]; then
    case "$CHAIN_SEP" in
        sep_prefixed) ;;
        *) error "chain_separator must be 'sep_prefixed', got '$CHAIN_SEP'" ;;
    esac
fi
