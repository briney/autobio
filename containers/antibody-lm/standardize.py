#!/usr/bin/env python3
"""Standardize antibody LM outputs into autobio schema format.

Handles both embedding mode (copies .npy files) and PLL mode (copies scores).
Reads metadata produced by run_antibody_lm.py from outputs/raw/ and writes
...
def standardize(workspace: Path) -> None:
    ...
    # Strip org prefix from model name for cleaner output
